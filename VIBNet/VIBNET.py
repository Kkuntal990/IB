...
class VIBNet(tf.keras.Model):
    def __init__(self,dr,BETA,classes,prior, **kwargs):
        ...
        x = tf.keras.layers.Conv2D(256, (1, 3), padding='valid', activation="relu", name="conv1",kernel_initializer='glorot_uniform')(x)
        x = tf.keras.layers.Dropout(dr)(x)
        x = tf.keras.layers.ZeroPadding2D((0, 2), data_format="channels_first")(x)
        x = tf.keras.layers.Conv2D(80, (2, 3), padding="valid", activation="relu", name="conv2",kernel_initializer='glorot_uniform')(x)
        x = tf.keras.layers.Dropout(dr)(x)
