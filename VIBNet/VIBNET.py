...
class VIBNet(tf.keras.Model):
    def __init__(self,dr,BETA,classes,prior, **kwargs):
        ...
        x = tf.keras.layersReshape(target_shape=(2,128,1,))(inp)
        x = tf.keras.layersZeroPadding2D((0,2), data_format="channels_first")(x)
        x = tf.keras.layersConv2D(256, (1, 3), padding='valid', activation="relu", name="conv1",kernel_initializer='glorot_uniform')(x)
        x = tf.keras.layersDropout(dr)(x)
        x = tf.keras.layersZeroPadding2D((0, 2), data_format="channels_first")(x)
        x = tf.keras.layersConv2D(80, (2, 3), padding="valid", activation="relu", name="conv2",kernel_initializer='glorot_uniform')(x)
        x = tf.keras.layersDropout(dr)(x)
        x = tf.keras.layersFlatten()(x)
        x = tf.keras.layersDense(512, activation='relu', name="mu",kernel_initializer='he_normal')(x)
        '''
