...
# %%
# dropout rate (%) 卷积层部分  https://keras-cn.readthedocs.io/en/latest/layers/convolutional_layer/#conv2d
dr = 0.5
# 这里使用keras的序贯模型  https://keras-cn.readthedocs.io/en/latest/models/sequential/

inp = keras.Input(shape=(in_shp,)).reshape(([1]+in_shp))
x = ZeroPadding2D((0,2), data_format="channels_first")(inp)
x = Conv2D(256, (1, 3), padding='valid', activation="relu", name="conv1", init='glorot_uniform', data_format="channels_first")(x)
x = Dropout(dr)(x)
x = ZeroPadding2D((0, 2), data_format="channels_first")(x)
x = Conv2D(80, (2, 3), padding="valid", activation="relu", name="conv2", init='glorot_uniform', data_format="channels_first")
x = Dropout(dr)(x)
x = Flatten()(x)
x = Dense(256, activation='relu', init='he_normal', name="dense1")(x)
x = Dropout(dr)(x)
mu, sigma = x[:,:,:, :128], x[:,:,:, 128:]
z = keras.Lambda(sample_z, output_shape=(128, ), name='z')([mu, sigma])
y = Dense(len(classes), init='he_normal', name="dense2")(z)
y = Activation('softmax')(y)
y = y.reshape((len(classes)))
...
filepath = "convmodrecnets_%s_0.5.wts.h5" % (
    name)  # 所要保存的文件名字，h5格式，不用写路径，默认在程序执行的文件夹内
